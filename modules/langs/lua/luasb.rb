...
if defined? JRuby
	...
	addCommand(@luasb["_VERSION"].tostring().downcase().delete(' ') ,:luasb,"Run Lua code in a Sandbox.")
	addCommand("resetlua", :luasb_reset, "Resets the Lua Sandbox.")
	addCommand("reset"+@luasb["_VERSION"].tostring().downcase().delete(' '), :luasb_reset, "Resets the Lua Sandbox.")
end
